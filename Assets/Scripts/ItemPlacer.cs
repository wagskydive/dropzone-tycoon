...
public class ItemPlacer : MonoBehaviour, ISpawnRequester
{
    ...
    {
        GameObject go = previousGameObjects[previousGameObjects.Count - 1];
    ...
    void DestroyPreviousList()
    {
        ...
        previousGameObjects.TrimExcess();
    }
    ...
    void PlaceObject(Vector3 position, Transform parent)
    {
        ...
        previousGameObjects[0].SetActive(false);
        previousGameObjects[0].transform.position = position;
        DestroyPreviousList();
        //spawner.Spawn(currentSpawnable, position);

        OnSpawnRequest?.Invoke(currentSpawnable, previousGameObjects[0].transform, parent);
